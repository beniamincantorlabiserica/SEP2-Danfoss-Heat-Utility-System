...
{
    public OptimizerManager(ResultManager resultManager, ExcelLoader excelLoader, int scenario)
    {
        var dataPerHours = excelLoader.GetData();

        foreach (var result in from data in dataPerHours let optimizer = new Optimizer(data.HourStart, data.HourEnd, data.Demand, data.ElectricityPrice, data.Period, scenario) select new ResultDataPerHour(optimizer.GetProductionUnits(), data.HourStart, data.HourEnd, data.Demand, data.ElectricityPrice, data.Period, optimizer.GetTotalCost()))
        {
